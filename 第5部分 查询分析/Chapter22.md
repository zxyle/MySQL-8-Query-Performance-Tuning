...
连接 1> 选择城市.*， 睡眠 （180） 从世界. 城市 ID = 130;

连接 2> 带读取锁的冲洗表;

连接 3> 选择 * 从世界. city ID = 3805;
...
current_statement： 选择城市.*， 睡眠 （180） 从城市 ID = 130
...
current_statement： 从 ID = 3805 的城市中选择 *
...
current_statement： 选择thd_id， conn_id， 状态， ...ession 命令 = "查询"
...
current_statement： 选择 *， 睡眠 （180） 从城市 ID = 130
...
连接 2> ALTER TABLE world.city ADD INDEX（名称）;
...
current_statement： 选择代码， 名称从世界. 国家代码 = "美国"
...
current_statement： 改变表世界. 城市添加索引 （名称）

statement_latency： 19.92米
...
current_statement： 选择thd_id， conn_id， 状态， ...查询'或trx_state = "活动"
...
从 sys.schema_table_lock_waits\G
...
waiting_lock_type： 独家

waiting_lock_duration： 交易

waiting_query： 改变表世界. 城市添加索引 （名称）
...
blocking_lock_duration： 交易

sql_kill_blocking_query： 杀死查询 7090

sql_kill_blocking_connection： 杀死 7090
...
sql_kill_blocking_query： 杀死查询 9

sql_kill_blocking_connection： 杀死 9
...
从 sys.schema_table_lock_waits

在哪里waiting_lock_type = "独家"

和waiting_pid<>blocking_pid\G
...
mysql> 选择blocking_pid， 计数（*）

从 sys.schema_table_lock_waits

哪里waiting_pid <> blocking_pid

按blocking_pidblocking_pid
...
mysql> 选择EVENT_ID， CURRENT_SCHEMA，
...
和NESTING_EVENT_ID = 113

和NESTING_EVENT_TYPE = "交易" = g
...
SQL_TEXT： 从世界选择 * 代码 = "美国"

2 rows in set (0.0036 sec)

mysql> 选择ATTR_NAME， ATTR_VALUE

从performance_schema.会话_连接_attrs
...
mysql> 选择 FORMAT_PICO_TIME （5749876163371648） 作为年龄;
...
哪里Variable_name "innodb_row_lock%"

或类型 = "InnoDB 指标 - 锁定";

+-------------------------------+--------+---------+

|Variable_name |价值 |已启用 |

+-------------------------------+--------+---------+

|innodb_row_lock_current_waits |0 |是

|innodb_row_lock_time |595876 |是

|innodb_row_lock_time_avg |1683 |是

|innodb_row_lock_time_max |51531 |是

|innodb_row_lock_waits |354 |是

|lock_deadlocks |0 |是

|lock_rec_lock_created |0 |否 |

|lock_rec_lock_removed |0 |否 |

|lock_rec_lock_requests |0 |否 |

|lock_rec_lock_waits |0 |否 |

|lock_rec_locks |0 |否 |

|lock_row_lock_current_waits |0 |是

|lock_table_lock_created |0 |否 |

|lock_table_lock_removed |0 |否 |

|lock_table_lock_waits |0 |否 |

|lock_table_locks |0 |否 |
...
查询确定，1 行受影响（0.0005 秒）

匹配行： 1 已更改： 1 警告： 0

连接 2> 设置会话innodb_lock_wait_timeout = 300;
...
连接 2> 更新 world. city Set 人口 = 人口 * 1.10 其中国家代码 = "澳大利亚";
...
mysql> 选择 * sys.innodb_lock_waits\ g
...
locked_table_schema： 世界

locked_table_name： 城市

locked_table_partition： 空

locked_table_subpartition： 空
...
waiting_query： 更新城市设置人口 + P...1.10 其中国家代码 = "澳大利亚"
...
哪里Variable_name = "lock_deadlocks"\ G
...
连接 1> 更新 world. city Set 人口 = 人口 * 1.10 其中国家代码 = "澳大利亚";
...
连接 2> 更新世界.国家设置人口 + 人口 = 146000 其中代码 = "澳大利亚";
...
连接 1> 更新 world. 国家设置人口 = 人口 * 1.1 其中代码 = "澳大利亚";

连接 2> 插入世界.城市价值 （4080， '达尔文'， '澳大利亚'， '北领地'， 146000）;
...
交易 6260，活动 62 秒启动索引读取

mysql 表在使用 1， 锁定 1

锁定等待 6 锁结构，堆大小 1136，30 行锁（s），撤消日志条目 14

MySQL 线程 ID 61， OS 线程句柄 22592， 查询 ID 39059 本地主机 ：：1 根更新

更新世界. 国家设置人口 = 人口 * 1.1 其中代码 = "澳大利亚"

（1） 持有锁：：

记录锁空间 ID 160 页没有 14 n 位 1368 索引国家代码表 'world'. '城市' trx id 6260 lock_mode X 锁间隙之前 rec

记录锁，堆号 652 物理记录：n_fields 2;紧凑格式;信息位 0

0： len 3;十六进制 415554;asc Aut;;

1： len 4;十六进制 800005f3;asc ;;

（1） 等待授予此锁：

记录锁空间 ID 161 页没有 5 n 位 128 索引主要表 '世界'. '国家' trx id 6260 lock_mode X 锁 rec， 但不是间隙等待

记录锁定，堆号 16 物理记录：n_fields 17;紧凑格式;信息位 0

0： len 3;十六进制 415553;asc 澳大利亚;;

1： len 6;十六进制 000000001875;asc u;;

2： len 7;十六进制 0200000122066e;asc \ n;;

3： len 30;十六进制 4175737472616c69612020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020asc 澳大利亚 ;（共 52 字节）;

4： len 1;十六进制 05;asc ;;

5： len 26;十六进制 4175737472616c696120616e64204e6577205a65616c616e6420;asc 澳大利亚和新西兰;;

6： len 4;十六进制 483eec4a;asc H> J;;

7： len 2;十六进制 876d;asc m;;

8： len 4;十六进制 812267c0;asc "g;;

9： len 4;十六进制 9a999f42;asc B;;

10： len 4;十六进制 c079ab48;asc y H;;

11： len 4;十六进制 e0d9bf48;asc H;;

12： len 30;十六进制 4175737472616c69612020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020asc 澳大利亚 ;（共 45 字节）;

13： len 30;十六进制 436f6e7374697475746f6e616c204d6f6e61726368792c204665646572; asc 宪法君主制， 费德勒;（共 45 字节）;

14： len 30;十六进制 456c697361626574682049492020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020阿斯克伊丽莎白二世 ;（共 60 字节）;

15： len 4;十六进制 80000087;asc ;;

16： len 2;十六进制 4155;asc Au;;
...
锁定等待 4 锁结构，堆大小 1136，2 行锁（s），撤消日志条目 2

MySQL 线程 ID 62， OS 线程句柄 2044， 查询 ID 39060 本地主机 ：：1 根更新

插入世界.城市价值 （4080， "达尔文"， "澳大利亚"， "北领地"， 146000）

*** (2) HOLDS THE LOCK(S):

记录锁空间 ID 161 页没有 5 n 位 128 索引主要表 '世界'. '国家' trx id 6261 lock_mode X 锁 rec， 但不是间隙
...
13： len 30;十六进制 436f6e737469747574696f6e616c204d6f6e6e61726368792c20466666665572;阿斯克君主立宪制，费德勒;（共 45 字节）;
...
记录锁空间 ID 160 页没有 14 n 位 1368 索引国家代码表 'world'. '城市' trx id 6261 lock_mode X 锁间隙之前 rec 插入意图等待
...
mysql> 选择SQL_TEXT， NESTING_EVENT_ID，

NESTING_EVENT_TYPE

从 performance_schema. events_语句_历史记录

地点THREAD_ID = PS_THREAD_ID （61）
...
NESTING_EVENT_TYPE： 空

2.行***************************************************************************************************

SQL_TEXT： 更新世界. 城市设置人口 + 人口 * 1.10 其中国家代码 = "澳大利亚"
...
SQL_TEXT： 更新世界. 国家设置人口 + 人口 * 1.1 其中代码 = "澳大利亚"
...
SQL_TEXT： 更新世界. 国家设置人口 + 人口 = 146000 其中代码 = "澳大利亚"
...
SQL_TEXT： 插入世界. 城市价值 （4080， '达尔文'， '澳大利亚'， '北领地'， 146000）
