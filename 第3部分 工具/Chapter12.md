...
https://doi.org/10.1007/978-1-4842-5584-1_12
...
是第二代命令行客户端，它通过支持 X 协议以及 Python 和 JavaScript 语言，与传统的命令行客户端相比非常突出。它还配备了几个实用程序，并且高度可扩展。这使得它不仅成为日常任务的重要工具，也成为调查性能问题的重要工具。

本章首先概述 MySQL 命令行程序提供哪些功能，包括内置帮助和丰富的提示。本章的第二部分介绍如何使用外部代码模块、报告基础结构和插件扩展 MySQL 命令行程序的功能。
...
本节将介绍安装 MySQL 外壳、调用它以及一些基本功能。但是，无法详细介绍 MySQL 命令行程序的所有功能。在使用 MySQL 雪壳时在线手册，以了解更多信息。
...
本章介绍了 MySQL 外壳。它最初概述了如何安装和使用 MySQL 命令程序，包括使用连接;SQL、Python 和 JavaScript 语言模式;内置帮助;和全局对象。本章的其余部分介绍 MySQL 命令行的自定义、报告和扩展 MySQL。
