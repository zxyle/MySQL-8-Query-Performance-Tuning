...
在 MySQL（和其他关系数据库）中创建表时，您可以为每个列指定数据类型。为什么不把所有东西都存储为字符串呢？毕竟，当这本书使用"42"时，它表示为字符串，为什么不只对每一项都使用字符串，并允许每列的各种值呢？这一点有其优点。这是NoSQL数据库工作方法的一部分（尽管还有更多），本书的作者看到了所有列被定义为 varchar(255) 字符串的表。为什么要费心使用整数、小数、浮数、日期、字符串等？有几个原因，这是本章的主题。

首先，将讨论对不同类型的值使用不同的数据类型的好处。然后，将概述 MySQL 中支持的数据类型。最后，将讨论数据类型如何影响查询性能以及如何选择列的数据类型。

## Why Data Types?

列的数据类型定义可以存储哪些类型的值以及如何存储值。此外，可能有与数据类型关联的元属性，例如大小（例如，用于字符串中字符数和最大字符数的字节数）和字符串字符集和排序规则。虽然数据类型属性可能看起来是不必要的限制，但它们也有其优点。这些好处包括
...
在其核心，数据类型定义允许的值类型。定义为整数数据类型的列只能存储整数值。这也是一种保障。如果您犯了一个错误，并尝试将值存储到与定义的数据类型不同的列中，则可以拒绝该值或转换该值。

**提示 是否将错误数据类型的值分配给列会导致 anerror 或正在转换的数据类型取决于您是否启用了 STRICT_TRANS_TABLES（对于事务存储引擎）和 STRICT_ALL_TABLES（对于所有存储引擎）SQL 模式，以及转换数据类型是否被视为安全。一些被视为安全的转换始终是不允许的，例如，将"42"转换为 42，反之亦然。建议始终启用严格模式，使 DML 查询在尝试不安全转换或截断数据时失败。**
...
关于数据类型和数据验证，还有一个注意事项。通常，存在与数据类型关联的属性。在最简单的情况下，您具有最大大小。例如，整数的大小可以是 1、2、3、4 或 8 字节。这会影响可存储的值范围。此外，整数可以签名或取消符号。更复杂的示例是字符串，它不仅限制它们存储的文本量，还需要一个字符集来定义数据的编码方式，以及一个排序规则来定义数据的排序方式。
...
SQL 模式设置为默认值，其中包括STRICT_TRANS_TABLES。除了主键之外，该表还有两列，其中一列是未签名的整数，另一列是 varchar（5），这意味着它最多可以存储五个字符。当尝试将字符串或负整数插入 val1 列时，将弹出该值，因为它无法安全地转换为未签名的整数。同样，尝试将包含六个字符的字符串存储到 val2 列中失败。但是，将字符串'42'存储到 val1 中，将整数 42 存储到 val2 中被认为是安全的，因此是允许的。
...
**提示 在表中提供文档的最佳方法是使用"注释"子句和 CHECK 约束。然而，这些通常不可见的不可预期的图表，这是数据类型有助于提供更好的心理图片，预期的数据类型**

性能主题方面，显式选择数据类型也有其好处。其中之一与值的存储方式有关。

### Optimized Storage

MySQL 不会以相同方式存储所有数据。选择给定数据类型的存储格式尽可能紧凑以减少所需的存储。例如，考虑值 123456。如果将其存储为字符串，则至少需要 6 个字节加上 1 个字节来存储字符串的长度。如果改为选择整数，则只需要 3 个字节（对于整数，所有值始终使用相同数量的字节，取决于列允许的最大存储空间）。此外，从存储读取整数不需要对值进行任何解释，1 而对于占星，需要使用其字符集解码值。

选择列的正确最大大小可以减少所需的存储量。如果需要存储整数，并且知道永远不需要超过 4 字节的存储的值，可以使用 int 数据类型，而不是使用 8 字节存储的 bigint。这是列所需的存储量的一半。如果您使用大数据，存储（和内存）节省可能会变得足够大，足以成为重要。但是，请注意不要过度优化。在许多情况下，更改列克的数据类型或大小需要重建整个表，如果表很大，则可能是一种昂贵的操作。这样，最好现在再使用一点存储空间来保存以后的工作。
...
并非所有数据类型都是平等的。在计算和比较中使用整数非常便宜，而存储的字节必须使用字符集解码的字符串相对昂贵。通过选择正确的数据类型，您可以显著提高查询的性能。特别是，如果您需要比较两列中的值（可能在不同的表中），请确保它们具有相同的数据类型，包括字符串的字符集和排序规则。否则，必须转换其中一个列中的数据，然后才能将其与其他列进行比较。

虽然理解为什么整数比字符串性能更好，但使一种数据类型的性能优于或更差的是什么相对复杂，取决于数据类型的实现方式（存储在磁盘上）。因此，性能的进一步讨论将推迟到下一节中对 MySQLdata 类型的演练之后。
...
- 正确排序需要了解两个值是否相等，或者值是否在给定范围内。这对于 WHERE 子句和联接条件如预期的那样工作至关重要。
- 创建索引时，排序用于确保 MySQL 快速找到具有要查找的值的行。下一章将详细介绍索引。
...
MySQL 中有 30 多种不同的数据类型。其中几个可以调整的大小，精度，以及它们是否接受签名的值。它首先看起来势不可挡，但如果将数据类型分组到类别中，您可以采取一种步步的方法为数据选择正确的数据类型。

MySQL中的数据类型可以视为以下类别之一的一部分：

- 数值：包括整数，固定精度的十进制类型，近似精度的浮点类型和位类型。
- 时间：包括年，日期，时间，日期时间和时间戳值。
- 字符串：包括二进制对象和带有字符集的字符串。
- JSON：JSON数据类型可以存储JSON文档。
- 空间：这些类型用于存储描述坐标系统中一个或多个点的值。
- 混合：MySQL有两种数据类型，都可以用作整数和字符串。

提示 MySQL 参考手册对其中的数据类型进行了全面的https://dev.mysql.com/doc/refman/8.0/en/data-types.htmland引用。

本节的其余部分将介绍数据类型并讨论其详细信息。

### Numeric Data Types

数字数据类型是 MySQL 支持的最简单数据类型。您可以在整数、固定精度十进制值和近似浮点值之间进行选择。
...
**提示切勿使用浮点数据类型来存储准确的数据，如货币存储。请改为使用精确的十进制数据类型。对于近似浮动点数据类型，绝不应使用相等 （+） 和不相等 （+） 运算符，因为比较两个近似值一般不会返回它们相等，即使它们本来是相等的。**
